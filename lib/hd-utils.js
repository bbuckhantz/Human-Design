...
export function centersFromGates(gates, channels) {
  ...
  // FIX: Convert channels to strings before splitting
  for (const ch of channels) {
    const [a,b] = String(ch).split("-");
...
export function buildChartJSON(input){
  ...
  // FIX: Convert channels to strings
  const channels = (input.channels && input.channels.length) 
    ? input.channels.map(ch => String(ch))
    : gatesToChannels(gates);
